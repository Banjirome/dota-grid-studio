...
- Генерация сетки по картинке: контуры референса автоматически превращаются в символы `. : - | / \ ( )` с настраиваемым шагом, чувствительностью и штриховкой.
...
### Генерация сетки по картинке

1. Перетащите картинку в окно (или вставьте через `Ctrl+V`, или нажмите **Trace image…** в меню).
2. При необходимости подвиньте и отмасштабируйте референс: итоговый арт займёт ровно его прямоугольник.
3. Нажмите **Generate grid from image** в инспекторе референса.
4. Подберите параметры в предпросмотре и нажмите **Insert**.

| Параметр | Что делает |
| --- | --- |
| Spacing | Расстояние между соседними символами в единицах Dota |
| Edge sensitivity | Насколько слабые контуры ещё считаются линиями |
| Dot density | Плотность точек-штриховки в тёмных (или светлых) областях без контуров |
| Symbols | Включение и вес каждого символа: вес выше 100% заставляет символ побеждать чаще, 0 отключает его |

Символы расставляются вдоль найденных контуров: `-` и `|` для горизонтальных и вертикальных штрихов, `/` и `\` для диагоналей, `(` и `)` для изогнутых вертикалей, `.` и `:` для коротких отрезков. Вставленные элементы добавляются в текущую сетку и остаются выделенными, поэтому один Undo убирает их целиком.
